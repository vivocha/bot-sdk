...
| `environment` | (optional) object                                                                               | Vivocha specific environment data,  sent by the platform. Currently, the `environment` object COULD have the following (optional) properties: `host`, `acct`, `hmac`, `campaignId`, `channelId`, `entrypointId`, `engagementId`, `contactId`, `token`. For a bot used in data collections of type `Bot`, the `environment` object DOES NOT contain neighter the `contactId` property (because it is a pre-contact task) nor the `token` property. |
